...
class shakti:
    ...
    async def execute_change_icon(self, token: str):
        ...
        for i, img in enumerate(images, 1):
            print(f"{i}: {img}")
        try:
            choice = int((await self.async_input("Choose icon number", 50)).strip()) - 1
    ...
    async def execute_change_guild_info(self, token: str, new_name: str = None, new_desc: str = None):
        if new_name is None:
            ...
            new_name = await self.async_input(f"New guild name (default: {default_name})", 50)
            new_name = new_name.strip() or default_name
        if new_desc is None:
            new_desc = await self.async_input("New guild description", 50)
    ...
    async def execute_give_admin(self, token: str):
        try:
            ...
            user_input = await self.async_input("User IDs (comma-separated) or 'all'", 50)
    ...
    async def execute_rename_channels(self, token: str):
        new_name = await self.async_input("New channel Name", 50)
    ...
    async def execute_rename_roles(self, token: str):
        new_name = await self.async_input("New role Name", 50)
        resp = await self._api_request("get", f"https://discord.com/api/v10/guilds/{self.guildid}/roles", token)
        if not resp or resp.status_code != 200:
            return 0
        roles = resp.json()
    ...
    async def execute_rename_emojis(self, token: str):
        new_name = await self.async_input("New emoji name (use {i})", 50)
    ...
    async def execute_unban_all(self, token: str):
        choice = await self.async_input("Unban specific user ID, or 'all'? ", 50)
    ...
    async def execute_dm_all(self, token: str):
        ...
        message = await self.async_input(f"DM message (default: {default_dm})", 50)
    ...
    async def execute_get_invite(self, token: str):
        try:
            platform = await self.async_input("Platform: [w]indows/[m]obile", 32)
    ...
    async def execute_guild_info(self, token: str):
        guild_id = await self.async_input("Enter guild ID", 50)
    ...
    async def _menu_animator(self):
        """Simple menu animator - reduced for small screens"""
        while self._animating:
            await asyncio.sleep(0.1)

    async def menu(self):
        """Main menu with all 41 options"""
        while True:
            try:
    ...
    <41> Get Bot Invite Link"""
            ...
            if ans in ["1", "01"]:
                ...
                days = int((await self.async_input("Prune days (1-30)", 50)).strip())
                if 1 <= days <= 30:
                    await self.execute_prune(days, token)
                    
            elif ans in ["4", "04"]:
                type_input = await self.async_input("Channel type [t/v]", 50)
                type_ = 2 if type_input.strip().lower() == 'v' else 0
                amount = int((await self.async_input("Amount", 50)).strip())
                ...
                amount = int((await self.async_input("Spam amount", 50)).strip())
                for _ in range(amount):
                ...
                nick = await self.async_input("New nickname", 50)
                ...
                new_guild = await self.async_input("Enter new Guild ID", 50)
                os.system("cls") if os.name == "nt" else os.system("clear")
                ...
                choice = await self.async_input("Timeout: [1]Day [2]Week [3]28Days", 50)
                ...
                choice = await self.async_input("[1]Grant admin [2]Toggle auto-admin", 50)
                if choice == "1":
                ...
                uid = await self.async_input("User ID to whitelist", 50)
                await self.add_to_whitelist(uid.strip())
                
            elif ans == "35":
                uid = await self.async_input("User ID to remove from whitelist", 50)
